...
- **正文期 AI 味检测 + 润色阶段** — `novel-write` 委托 `ai-smell-guard` agent 执行 27 项文本质检与定点修正，agent 完成后主线程再做独立润色
- **强制返修环节** — 交付后用户在正文中标记问题点（加粗=润滑、斜体=合并、删除线=清理重写、行内代码=扩写），`revision-handler` agent 按标记规则处理，可循环多轮
...
| `/booming`         | booming、太平了、不够炸 | 独立触发的剧情爆破模式，但主要给 `novel-discuss` 用；默认给 3 套高烈度爆破走向，至少一套必须真正掀桌，用户确认后再交给 `/novel-plan` 落正式大纲 |
...
| `/novel-write`     | 写章节、创作正文       | 默认全风格逐个调用 5 个 writer subagent，正文阶段细切为固定 20 个剧情点；5 个风格稿都显性保留剧情点标题，最终稿按剧情点逐点择优并标记来源风格，合并后先做最终稿一致性校验，再委托 `ai-smell-guard` agent 做 27 项 AI 味检测与定点修正，最后主线程再做独立润色；交付后强制进入返修环节，用户标记问题点后由 `revision-handler` agent 按规则处理 |
...
| `revision-handler` | 为 `novel-write` 处理人工返修标记，按规则执行润滑/合并/清理重写/扩写 |
...
/novel-plan          →  先澄清每个小剧情点的 5W1H，再生成双层大纲（剧情思路卡 + 可写场景纲要）+ 内置规划期强制反思与爽点检查
    ↓
/novel-write         →  正文阶段细切为固定 20 个剧情点 + 默认全风格逐个调用 5 个 writer subagent + 最终稿按剧情点逐点择优并标记来源风格 + 合并后先做一致性校验，再委托 ai-smell-guard agent 做 AI 味检测，最后主线程做独立润色
    ↓
生成最终稿          →  自动合并生成最终稿，并保留各风格中间稿供回看
    ↓
返修                →  用户在正文中标记问题点，revision-handler agent 按标记规则处理（润滑/合并/清理重写/扩写），可循环多轮
    ↓
/novel-sync          →  同步更新知识图谱
    ↓
...
9. 最终稿合并后，先做最终稿一致性校验，再委托 `ai-smell-guard` agent 做正文 AI 味检测与定点修正（27 项检测，含过渡词清理、冗余描写删除、短句合并重写等）；agent 完成后，主线程再做一轮独立润色，把物品、环境、人物描写升级成更优美、精美、华丽、有高级感的表达，并把人物行为描写升级成更豪迈、精准、刺激、带武侠风味的表达
10. 交付后强制进入返修环节：用户在正文中标记问题点（加粗=润滑、斜体=合并、删除线=清理重写、行内代码=扩写），`revision-handler` agent 按标记规则处理，可循环多轮，直到用户满意或确认无需返修后才进入同步
