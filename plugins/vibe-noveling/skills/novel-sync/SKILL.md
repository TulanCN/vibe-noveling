---
name: novel-sync
description: 当章节已通过当前工作流检查并经人工确认，需要回写知识图谱与剧情摘要时使用。
when_to_use: |
  适用于“同步”“sync”“更新状态”“章节完成同步”“更新知识图谱”“更新past”“同步一下”“已合并正文”等请求。
  仅在正文通过返修确认后执行；正文创作改用 `novel-write`；返修改用 `novel-revise`。
---
...
在章节**通过当前工作流中的一致性检查后**，同步更新知识图谱、剧情摘要、伏笔状态，确保项目状态与正文进度一致。

## 工作流位置

```
novel-write（创作正文 + 内置一致性检查）
      ↓
novel-revise（返修）
      ↓
人工确认（用户确认）
...
**重要**：必须先完成返修（`/novel-revise`）并通过人工确认后再执行同步。
...
- 读取最新章节大纲 `chapters/vol-{volume_padded}/ch-{chapter_padded}/大纲.md` → 当前章节标题
  - 若标题仍为“待定”，则在创作进度中写“标题待定”或只保留章节号
  - 不要把未确认标题写成当前章节标题
