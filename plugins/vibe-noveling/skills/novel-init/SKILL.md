---
name: novel-init
description: 当用户要初始化一个新的小说项目，建立目录结构和基础创作契约时使用。
when_to_use: |
  适用于“初始化小说”“新建小说项目”“开始写小说”“创建小说”“帮我搭建小说项目”“开始一个新的故事”等请求。
  如果项目已经存在并进入后续创作阶段，改用其他 skills。
---
# 小说项目初始化

## 何时使用

- 用户还没有项目骨架，需要先搭好目录和默认配置
- 用户要开始一个新的故事，而不是继续已有项目

## 调用边界

- 只负责创建项目基础设施和初始契约
- 已有项目的规划、写作、同步改由对应 skills 处理
...
创建 `CLAUDE.md` 文件，帮助后续创作时快速了解项目结构，并明确项目级默认创作风格基线：

```markdown
# CLAUDE.md

本文件为 Claude Code 提供项目结构说明和项目级写作契约，避免每次创作时重复探索。
...
- 项目默认按中国男频网络小说思路创作
- 主角默认男性
- 主角的关键行为的终极目标，都是为装逼服务
- 感情戏优先用于放大主角特殊待遇，而不是平均分配抒情篇幅
- 与装逼链条无关的剧情默认砍掉、合并或压缩

### 默认爽点循环

`蓄势 -> 受压/被轻视/形成对比 -> 主角出手或亮底牌 -> 他人震动/改变态度/被打脸 -> 主角获得资源、地位、关系或名望上的兑现`

如果当前章节只是过桥章，也要明确说明它是在为下一次装逼兑现做什么蓄势。
...
- `大纲.md` - 章节大纲
- `上下文.md` - 章节上下文缓存
- `Opus试写.md` / `Opus报告.md` - 写作测试稿与反推报告
- `海明威.md` / `卖报小郎君.md` / `会说话的肘子.md` / `大仲马.md` / `吐槽口语风.md` - 风格中间稿
- `正文.md` - 最终正文
...
💡 小贴士：
   ...
   - 默认创作风格基线也会写入 CLAUDE.md，供后续规划和正文创作持续复用
